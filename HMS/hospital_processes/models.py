...
class CheckIn(models.Model):
    checkin_id = models.BigAutoField(max_length=8, primary_key=True)
    ...
    staff = models.CharField(Staff) #make foreign key once we get staff model
...
class Appointment(models.Model):
    appointment_id = models.BigAutoField(max_length=8, primary_key=True)
...
class DiagnosticTests(models.Model):
    ...
    test_id = models.BigAutoField(max_length=8, primary_key=True)
    patient_id = models.ForeignKey(Patient, on_delete=models.CASCADE)
...
class Surgeries(models.Model):
    condition_id = models.ForeignKey(MedicalCondition, on_delete=models.CASCADE)
    surgery_id = models.BigAutoField(max_length=8, primary_key=True)
    patient_id = models.ForeignKey(Patient, on_delete=models.CASCADE)
...
class MedicalNotes(models.Model):
    note_id = models.BigAutoField(max_length=8, primary_key=True)
    patient_id = models.ForeignKey(Patient, on_delete=models.CASCADE)
